...
//   def createSocketConnection(s: Socket) = {
...
//   def createSockoConnection(s: WebSocketFrameEvent) = {
//     val connection: ActorRef = networkSystem.actorOf(Props(new SockoConnection(nextId, s, interpreter)), name = (new UID()).toString)
//     connections = connections + (nextId -> connection)
...
//   def receive = {
//     // case CreateSocketConnection(s) => createSocketConnection(s)
//     case CreateSockoConnection(s) => createSockoConnection(s)
